% File: bike_inertia_jason_browser_mod.m
...
% TU Delft.
% Updates: Changes the legs to separate cylinders.

clear
close all
clc
warning off MATLAB:fzero:UndeterminedSyntax
%-------------------Frame Parameters (Batavus Browser)
df      = 0.684;                         % front wheel diameter [m]
dr      = 0.684;                         % rear wheel diameter [m]
rf      = df/2;                          % front wheel radius [m]
rr      = dr/2;                          % rear wheel radius [m]
wb      = 1.12;                          % wheel base [m]
hb      = 0.295;                         % bottom bracket height [m]
lcs     = 0.46;                          % chain stay length [m]
alphast = 68.5*pi/180;                   % seat tube angle [rad]
lst     = 0.53;                          % seat tube length [m]
tr      = 0.055;                         % trail [m]
lambda  = 68.5*pi/180;                   % head tube angle [rad]
fo      = rf*cos(lambda)-tr*sin(lambda); % fork offset [m]
lf      = 0.455;                         % fork length [m]
lsp     = 0.24;                          % seat post length [m]
wr      = 0.13;                          % rear hub width [m]
wf      = 0.10;                          % front hub width [m]
whb     = 0.58;                          % handle bar width [m]
lhb     = 0.19;                          % handle bar length [m]
hs      = 0.25;                          % stem height [m]
...
alphah  = 82.9*pi/180;                   % hunch angle [rad]
