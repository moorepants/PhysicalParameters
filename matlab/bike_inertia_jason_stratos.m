% File: bike_inertia_jason_stratos.m
% Date: March 24, 2010
% Author: Jason Moore
% Description: Determines the critical speeds for the weave and capsize
% eigenmodes of a simplified bicycle model used in control experiments at 
% TU Delft. In this case the Batavus Stratos Deluxe and Jason as the rider. 

clear
close all
clc
warning off MATLAB:fzero:UndeterminedSyntax
%-------------------Frame Parameters (Stratos Deluxe)
df      = 0.334*2;                         % front wheel diameter [m]
dr      = 0.338*2;                         % rear wheel diameter [m]
rf      = df/2;                          % front wheel radius [m]
rr      = dr/2;                          % rear wheel radius [m]
wb      = 1.037;                          % wheel base [m]
hb      = 0.290;                         % bottom bracket height [m]
lcs     = 0.445;                          % chain stay length [m]
alphast = 75*pi/180;                   % seat tube angle [rad]
lst     = 0.480;                          % seat tube length [m]
tr      = 0.056;                         % trail [m]
lambda  = 73.1*pi/180;                   % head tube angle [rad]
fo      = rf*cos(lambda)-tr*sin(lambda); % fork offset [m]
lf      = 0.455;                         % fork length [m]
lsp     = 0.195;                          % seat post length [m]
wr      = 0.13;                          % rear hub width [m]
wf      = 0.10;                          % front hub width [m]
whb     = 0.58;                          % handle bar width [m] (this isn't the actual value)
lhb     = -0.090;                          % handle bar length [m]
hs      = 0.190;                          % stem height [m]
...
ele(21,1:7) = { 13 15 'torso'            'rprism'   'framex'  (cch-2*lss+2/pi*cua)/(pi-2)  lss-cua/pi};
ele(22,1:7) = { 25 25 'head'             'sphere'   'framex'  rh       0      };
ele(23,1:7) = { 21 23 'left upper arm'   'cylinder' 'framex'  cua/2/pi 0      };
ele(24,1:7) = { 22 24 'right upper arm'  'cylinder' 'framex'  cua/2/pi 0      };
ele(25,1:7) = { 19 23 'left lower arm'   'cylinder' 'framex'  cla/2/pi 0      };
ele(26,1:7) = { 20 24 'right lower arm'  'cylinder' 'framex'  cla/2/pi 0      };
%-------------------Draw 2D Bicycle
figure(1)
...
% end
% hold off
%-------------------Construct the Stability Matrix and Calculate Eigenvalues for Various Speeds
vmax = 10; % max foward velocity of the rear wheel to be calculated
n = 1000; % number of iterations
for i=1:n+1  
    v(i) = (i-1)/n*vmax; % ith velocity
    bench_par(20)=v(i);
    [S,D]=whip_eig(bench_par);
...
plot(v,weave(1,:),'b')
plot(v,weave(2,:),'b')
plot(v,weave_imag(1,:),'b:')
plot(v,weave_imag(2,:),'b:')
plot(v,capsize,'r')
plot(v,caster,'y')
% legend('Weave','Weave','Capsize','Caster')
plot(v,zeros(length(v),1),'k') %plot horizontal line at zero
hold off
%-------------------Calculate Critical Speeds
% find the Speed at which the weave mode becomes stable
...
vw=mean([v(index-1),v(index)]) % weave critical Speed
% find the Speed at which the capsize mode becomes unstable
...
vc=mean([v(index-1),v(index)]) % capsize critical Speed
%-------------------Plot the Critical Speeds
hold on
plot(vw,0,'ok',zeros(1,20)+vw,-19:1:0,'k',vc,0,'ok',zeros(1,20)+vc,-19:1:0,'k')
hold off
%-------------------Plot real parts of the eigenvalues vs. speed
figure(10)
hold on
box on
axis([0 10 -10 10])
title('Eigenmodes')
xlabel('Velocity [m/s]')
ylabel('Real Parts of the Eigenvalues [1/s]')
plot(v,weave(1,:),'b')
plot(v,weave(2,:),'b')
plot(v,capsize,'r')
plot(v,caster,'g')
